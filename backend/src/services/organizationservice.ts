      ...
      return new Promise((resolve, reject) => {
        ...
        .then((organization: any) => {
          this.eventService.getEvent(eventId).then((event: any) => {
            ...
            let availableProducts = this.eventService.getEventProducts(eventId)
                                    .then((products: any) => {
                                      return products.map( (product: any) => {
                                        return( {name: product.name, id: product.id, price: product.price, discounts: product.discounts });
                                      });
                                    });
            //No idea why this nees the dual promise unwapping
            Promise.all(platoonPromises)
            .then((resolvedPlatoons) => {
              Promise.all([event, availableProducts, resolvedPlatoons])
              .then((result: any) => {
